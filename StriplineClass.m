classdef StriplineClass
    ...
    methods (Access = public,Static)
        ...
        function [ConductorAttenuation] = getConductorAttenuation(StriplineClass.getStriplineWidth(relative_permittivity, characteristicImpedance, substrateThickness), relative_permittivity, surface_resistance, substrateThickness, conductorThickness, characteristicImpedance)
            ...
            B = 1 + ((substrateThickness * 10^(-2)) / (0.5 * (StriplineClass.getStriplineWidth(relative_permittivity, characteristicImpedance, substrateThickness) * 10^(-2)) + 0.7 * (conductorThickness * 10^(-3)))) * (0.5 + 0.414 * (conductorThickness * 10^(-3)) / (StriplineClass.getStriplineWidth(relative_permittivity, characteristicImpedance, substrateThickness) * 10^(-2)) + (1 / (2 * pi) * log(4 * pi * (width * 10^(-2)) / (conductorThickness * 10^(-3)))));
            if sqrt(relative_permittivity) * characteristicImpedance < 120
